...
function write(type: string, packet: any) {
  return nbt.proto.createPacketBuffer(type, packet);
}

function read(type: string, buffer: any) {
...
const testData = [
  ...
  },
  {
    ...
    data: [
      ...
      {
        buffer: Buffer.from([8, 0, 0, 0, 2, 0, 5, 0x48, 0x65, 0x6c, 0x6c, 0x6f, 0, 5, 0x57, 0x6f, 0x72, 0x6c, 0x64]),
  ...
  },
  {
    ...
    data: [
      {
        buffer: Buffer.from([1, 0, 2, 0x61, 0x61, 1, 9, 0, 2, 0x62, 0x62, 1, 0, 0, 0, 3, 1, 2, 3, 0]),
        value: {
      ...
      {
        buffer: Buffer.from([1, 0, 2, 0x63, 0x63, 2, 0]),
        value: {
...
describe('nbt.Writer', function () {
  testData.forEach(function (typeTest: NBTSampleData) {
    it('writes ' + typeTest.description, function () {
      typeTest.data.forEach(function (test: any) {
        expect(write(typeTest.type, test.value)).toStrictEqual(test.buffer);
      });
    });
  });
});
...
describe('nbt.Reader', function () {
  testData.forEach(function (typeTest: NBTSampleData) {
    it('reads ' + typeTest.description, function () {
      typeTest.data.forEach(function (test: any) {
        expect(read(typeTest.type, test.buffer)).toStrictEqual(test.value);
      });
    });
  });
});
